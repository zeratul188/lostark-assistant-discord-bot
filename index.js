...
인증받는 방법 :
\`\`\`
1. 아래 "인증받기" 버튼을 클릭해 먼저 "인증자" 역할을 받습니다.
2. 인증을 받은 후 카테고리에 "직업설정", "별명변경"이 있습니다. 해당 채널로 이동 후 인증을 진행하시면 됩니다.
3. 직업설정(채널)에서 직업 역할 받습니다.
4. 별명변경(채널)에서 별명을 변경하면 "사용자" 역할을 부여받으면 서버 이용을 할 수 있습니다.
\`\`\`
    ...
    collector.on('collect', async i => {
        if (i.customId === 'btnConfirm') {
            ...
            if (isDouble) {
                ...
                client.channels.cache.get('945891054408847470').send(`\`\`\`
${i.member.user.username}(${i.member.id})님이 \"사용자\" 권한을 부여받았습니다.
\`\`\``);
                await i.reply({
                    content: `\"인증자\" 권한을 획득하셨습니다.
